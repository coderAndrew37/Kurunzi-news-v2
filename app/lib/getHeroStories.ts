  ...
  *[_type == "hero"][0]{
    items[]{
      article->{
        ...
        // ✅ FIX: article has categories[] (array)
      ...
      overrideTitle,
      customImage
    } | order(article.publishedAt desc)[0..3]
  }
`);
  ...
  return validItems
    .filter((item) => item.article.category?.slug) // ⛑ safety
    .map((item) => ({
      id: item.article._id,
      ...
      category: item.article.category, // ✅ GUARANTEED
